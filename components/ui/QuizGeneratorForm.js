...
import { Settings, BookOpen, Target, BarChart2, Clock, List, Sparkles } from 'lucide-react';
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-3 flex items-center">
            ...
            年级水平 <span className="text-red-400 ml-1">*</span>
          </label>
          <select
            value={formData.grade_level}
            onChange={(e) => handleChange('grade_level', e.target.value)}
            className={`w-full px-4 py-3 bg-[#0f172a]/50 border rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all ${errors.grade_level ? 'border-red-500/50' : 'border-white/10 focus:border-cyan-500/50'
              }`}
          >
            <option value="" className="bg-[#0f172a]">请选择年级</option>
          ...
          {errors.grade_level && (
            <p className="mt-2 text-sm text-red-400">{errors.grade_level}</p>
          )}
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-3 flex items-center">
            ...
            学科选择 <span className="text-red-400 ml-1">*</span>
          </label>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {subjectOptions.map(option => (
              <button
                ...
                className={`p-3 border rounded-xl text-sm font-medium transition-all duration-200 flex items-center justify-center ${formData.subject === option.value
                    ? 'border-cyan-500 bg-cyan-500/20 text-cyan-300 shadow-[0_0_15px_rgba(6,182,212,0.2)]'
                    : 'border-white/10 bg-[#0f172a]/30 text-slate-400 hover:bg-white/5 hover:text-white hover:border-white/20'
                  }`}
              >
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-3 flex items-center">
            ...
            自评水平 <span className="text-red-400 ml-1">*</span>
          </label>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {levelOptions.map(option => (
              <label
                key={option.value}
                className={`block p-4 border rounded-xl cursor-pointer transition-all duration-200 ${formData.self_assessed_level === option.value
                    ? 'border-emerald-500 bg-emerald-500/10 shadow-[0_0_15px_rgba(16,185,129,0.1)]'
                    : 'border-white/10 bg-[#0f172a]/30 hover:bg-white/5 hover:border-white/20'
                  }`}
              >
                <div className="flex items-start">
                  ...
                  <div className="flex-1">
                    <div className={`text-sm font-bold mb-1 ${formData.self_assessed_level === option.value ? 'text-emerald-400' : 'text-white'
                      }`}>
                      {option.label}
                    </div>
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ml-3 mt-0.5 ${formData.self_assessed_level === option.value
                      ? 'border-emerald-500'
                      : 'border-slate-600'
                    }`}>
          ...
          {errors.self_assessed_level && (
            <p className="mt-2 text-sm text-red-400">{errors.self_assessed_level}</p>
          )}
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-3 flex items-center">
            ...
            学习目标 <span className="text-red-400 ml-1">*</span>
          </label>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {goalOptions.map(option => (
              <label
                key={option.value}
                className={`block p-4 border rounded-xl cursor-pointer transition-all duration-200 ${formData.learning_goal === option.value
              ...
              >
                <div className="flex items-start">
                  ...
                  />
                  <div className="flex-1">
                    <div className={`text-sm font-bold mb-1 ${formData.learning_goal === option.value ? 'text-pink-400' : 'text-white'
                      }`}>
                      {option.label}
                    </div>
                    <div className="text-xs text-slate-400 leading-relaxed">
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ml-3 mt-0.5 ${formData.learning_goal === option.value
                      ? 'border-pink-500'
                      : 'border-slate-600'
                    }`}>
          ...
          {errors.learning_goal && (
            <p className="mt-2 text-sm text-red-400">{errors.learning_goal}</p>
          )}
        </div>

        {/* 高级选项 */}
        <details className="group">
          <summary className="cursor-pointer text-sm font-medium text-cyan-400 hover:text-cyan-300 flex items-center select-none">
            <Sparkles className="w-4 h-4 mr-2" />
            高级选项 (可选)
          </summary>
          <div className="mt-6 space-y-6 pl-6 border-l-2 border-white/10">
            {/* 测试时长 */}
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-3 flex items-center">
              ...
              <select
                ...
                className="w-full px-4 py-3 bg-[#0f172a]/50 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all"
              >
                <option value="10" className="bg-[#0f172a]">10分钟（快速测试）</option>
                <option value="15" className="bg-[#0f172a]">15分钟（标准测试）</option>
                <option value="20" className="bg-[#0f172a]">20分钟（深度测试）</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-4 py-3 bg-[#0f172a]/50 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all"
              >
                <option value="auto" className="bg-[#0f172a]">自动调整（推荐）</option>
                <option value="8" className="bg-[#0f172a]">8题（简单测试）</option>
                <option value="12" className="bg-[#0f172a]">12题（标准测试）</option>
                <option value="16" className="bg-[#0f172a]">16题（完整测试）</option>
              </select>
            </div>
          </div>
        </details>
        ...
        <motion.button
          type="submit"
          disabled={loading}
          className={`w-full py-4 px-6 rounded-xl font-bold text-lg text-white transition-all shadow-lg ${loading
